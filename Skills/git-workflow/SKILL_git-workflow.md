...
**Target branch**: `development` (Default for all feature, fix, and refactor branches)
...
- `--feature-branch`: Use feature branch workflow (mandatory in goTOV)
...
- Prompt: "Strictly enforce goTOV Git Workflow: checkout and pull latest from `development`. Create a descriptive feature branch. Ensure branch name follows pattern (feature|fix|refactor)/<description>. Current branch: [insert branch], target: `development`. Verify no conflicts with target branch."
